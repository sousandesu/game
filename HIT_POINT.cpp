...
void HIT_POINT::draw()
{
	for (int i = 1; i < game()->container()->data().hitpoint.numHitPointContainer + 1; i++) {
		if (game()->characterManager()->player()->hp() >= i*2 ) {
			image(Hit_point.fullImg, Hit_point.px + Hit_point.offSetPx * i, Hit_point.py);
		}
		else if (game()->characterManager()->player()->hp() == i * 2 - 1)
		{
			image(Hit_point.halfImg, Hit_point.px + Hit_point.offSetPx * i, Hit_point.py);
		}
		else
		{
			image(Hit_point.emptyImg, Hit_point.px + Hit_point.offSetPx * i, Hit_point.py);
		}
	}
}
